...
class ChatRequest(BaseModel):
    ...
    model: str = Field(default=settings.DEFAULT_MODEL, description="Model name", example=settings.DEFAULT_MODEL)
    messages: List[ChatMessage] = Field(..., description="List of messages", example=[ChatMessage(role="user", content="Hello!")])
...
class ChatResponse(BaseModel):
    ...
    response: str = Field(..., description="Response from the model", example="Hello! How can I assist you?")
...
class ChatSection(BaseModel):
    ...
    created_at: str = Field(..., description="Creation timestamp", example="2023-10-01T12:00:00Z")
    model: str = Field(..., description="Model used for the chat", example=settings.DEFAULT_MODEL)
